...
app.post("/service/:service/critical", async (req, res) => {
  await notifier.tgCritical(
    `${req.params.service}:\t${
      req.body.message ?? "no_message"
...
app.post(
  "/analytics/platform/:platform/event/:event/user_id/:user_id",
  async (req, res) => {
...
app.post(
  "/metrics/platform/:platform/event/:event/user_id/:user_id",
  async (req, res) => {
    await metrics.write(
      req.params.platform,
      req.params.event,
